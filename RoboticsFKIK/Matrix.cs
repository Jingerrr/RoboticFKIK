...
namespace RoboticsFKIK
{
    ...
    public class Matrix
    {
        ...
        /// 用行数列数初始化矩阵
        /// </summary>
        /// <param name="_rows">矩阵行数</param>
        /// <param name="_cols">矩阵列数</param>
        ...
        /// 得到子矩阵
        /// </summary>
        /// <param name="fromRowIndex">子矩阵左上角行号</param>
        /// <param name="fromColIndex">子矩阵左上角列号</param>
        /// <param name="toRowIndex">子矩阵右下角行号</param>
        /// <param name="toColIndex">子矩阵左右下列号</param>
        /// <exception cref="Exception"></exception>
        public Matrix getChildMatrix(int fromRowIndex,int fromColIndex,int toRowIndex, int toColIndex)
        {
            if (fromRowIndex>=0&&fromRowIndex <= toRowIndex && toRowIndex < Rows &&
                fromColIndex >= 0 && fromColIndex <= toColIndex && toColIndex < Cols)
            {
                Matrix childM=new Matrix(toRowIndex- fromRowIndex+1, toColIndex- fromColIndex+1);
                int count = 0;
                for (int i = fromRowIndex; i <= toRowIndex; i++)
                {
                    for (int j = fromColIndex; j <= toColIndex; j++)
                    {
                        childM.element[count++] = this[i, j];
                    }
                }
                return childM;
            }
            ...
            {
                throw new Exception("子矩阵范围超过父矩阵");
            }
        }

        public Matrix getRow(int rowInd)
        {
            return getChildMatrix(rowInd, 0, rowInd, cols - 1);
        }
        public Matrix getCol(int colInd)
        {
            return getChildMatrix(0, colInd, rows-1, colInd);
        }
        ...
        public override string ToString()
        {
            ...
            for (int i = 0; i < Rows; i++)
            {
                for (int j = 0; j < Cols; j++)
                {
                    str += Math.Round(this[i, j],3).ToString();
                    if (j != Cols - 1)
                        str += " ";
                    else if (i != Rows - 1)
        ...
        {
            public Identity(int dimension) :base(dimension, dimension)
            {
                this.rows = dimension;
                this.cols = dimension;
                this.element = new double[dimension * dimension];
                for (int i = 0; i < rows; i++)
                {
                   element[i*dimension+i] = 1;
                }
            }
        }
        ...
        {
            public Vector(double[] m) : base(m.Length,1)
            {
